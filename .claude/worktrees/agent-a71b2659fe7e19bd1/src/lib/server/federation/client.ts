...
export function joinFederation(inviteToken: string): void {
	...
	const { id: societyId, handle: societyHandle, name } = society;
	...
	enqueueFederationMessage(type, societyHandle, { societyId, name, inviteToken, publicKey: keypair.public_key, signature }, { id, timestamp });
